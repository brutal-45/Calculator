...
function BrutalToolsLogo({ size = 36 }: { size?: number }) {
  ...
  return (
    ...
    >
      ...
      />
      <svg viewBox="0 0 64 64" fill="none" className="relative z-10 w-full h-full" aria-hidden="true">
        <defs>
        ...
        <rect x="3" y="3" width="58" height="58" rx="16" fill={`url(#${id}-bg)`} stroke={`url(#${id}-border)`} strokeWidth="1.5" filter={`url(#${id}-shadow)`} />
        <rect x="5" y="5" width="54" height="54" rx="14" fill="none" stroke="white" strokeOpacity="0.05" strokeWidth="0.5" />
        <path d="M18 46V18h12c3.3 0 6 2.7 6 6s-2.7 6-6 6h-12m0 0h13c3.3 0 6 2.7 6 6s-2.7 6-6 6H18" fill="none" stroke={`url(#${id}-letter)`} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />
        <path d="M37 12L28 30h6.5L26 52L44 26h-7.5L41 12H37Z" fill={`url(#${id}-bolt)`} filter={`url(#${id}-glow)`} />
        <path d="M37.5 16L31 29h5l-6.5 18L41 27h-5.5L39 16h-1.5Z" fill={`url(#${id}-bolt-inner)`} opacity="0.45" />
        <ellipse cx="32" cy="20" rx="22" ry="14" fill={`url(#${id}-shine)`} />
...
export function CalculatorApp() {
  const {
    mode, setMode, setHistory, history,
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-500 mt-0.5 font-medium">
                  by{' '}
                  <span className="text-shimmer font-bold">BrutalTools</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Sheet open={historyOpen} onOpenChange={setHistoryOpen}>
                <SheetTrigger asChild>
                  <button
                    className="relative flex items-center gap-1.5 px-2.5 py-2 rounded-xl bg-zinc-800/50 hover:bg-zinc-700/60 text-zinc-400 hover:text-zinc-200 text-sm transition-all border border-white/[0.05] hover:border-white/[0.1] cursor-pointer"
                  ...
                  >
                    ...
                    {history.length > 0 && (
                      <motion.span
                        ...
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] flex items-center justify-center rounded-full bg-emerald-500 text-[9px] font-bold text-white px-1 shadow-sm shadow-emerald-500/30"
                      >
                ...
                <SheetContent side="right" className="w-full sm:w-[380px] bg-zinc-950/95 backdrop-blur-xl border-white/[0.06] p-0">
              ...
              <Sheet>
                ...
                <SheetContent side="right" className="w-full sm:w-[400px] bg-zinc-950/95 backdrop-blur-xl border-white/[0.06] p-0">
